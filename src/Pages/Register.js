      ...
      <div data-aos="fade-up"
        ...
        <div className="left-login l-login dark:bg-slate-800 rounded-tl-3xl rounded-bl-3xl">
          ...
          <form onSubmit={handlesubmit} className="below-name mt-5 ml-10">
            ...
            <input onChange={(e) => { email.current.value = e.target.value }} autoComplete='false' className='bg-gray-200 dark:bg-slate-500  dark:placeholder:text-white' type="email" name="name" id
              ="name" placeholder='Email' ref={email} />
            <label className='mt-2 font dark:text-white' htmlFor="name">USERNAME</label>
            <input onChange={(e) => { name.current.value = e.target.value }} autoComplete='false' className='bg-gray-200 dark:bg-slate-500 dark:placeholder:text-white' type="text" name="emial" id
              ="name" placeholder='User Name' ref={name} />
            <label className='font dark:text-white mt-3' htmlFor="pass ">PASSWORD</label>
            <input onChange={(e) => { pass.current.value = e.target.value }} autoComplete='false' className='bg-gray-200 dark:bg-slate-500 dark:placeholder:text-white' type="password" name="pass" id="pass" placeholder='Password' ref={pass} />
